    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       //        processRequest(request, response);
        try {
            ...
            String sql = "select * from tb_login where username='" + email + "' and password='" + pwd + "'";
            rs = db.FetchData(sql);
            HttpSession session = request.getSession();
          
            if (rs.next()) {
                int regid=rs.getInt(4);
                ...
                response.sendRedirect("login.jsp?err");
                // out.print("<script>document.getElementById('msg').innerHTML="+email+";</script>");.forward(request, response);
