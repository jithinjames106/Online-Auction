...
public class AddCategory extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sql = "insert into tb_auction_category values(null,'" + category + "')";
            System.out.println(sql);
            int row = db.UpdateQuery(sql);
            if (row > 0) {
                // response.sendRedirect("AddCategory.jsp?suc");
                request.getRequestDispatcher("AddCategory.jsp").include(request, response);
                out.print("<script>document.getElementById('wer').innerHTML='Successulyy added';</script>");
            } else {
                request.getRequestDispatcher("AddCategory.jsp").include(request, response);
                out.print("<script>document.getElementById('wer').innerHTML='Error adding category';</script>");
            }
            db.Close();
